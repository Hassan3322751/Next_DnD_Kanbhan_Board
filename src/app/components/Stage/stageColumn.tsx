...
import { addTask, deleteTask, getTask, patchTask, getTasksByStage } from "@/Store/tasksSlice"; // Adjust the import based on your store structure
import DropArea from "../DropArea/DropArea";

const StageColumn = ({ id, tasks, stages, title, handleDelete, activeCard, setActiveCard, stageData, onDrop }) => {
    ...
    // useEffect(() => {
    //     console.log(stage)
    //     dispatch(getTasksByStage({ stageId: id, page: currentPage }));
    // }, []);
    // const { tasks, hasMore, loading } = useSelector((state) => state.tasksData); // Get tasks from Redux store
    ...
    //     await dispatch(patchTask(newTask));
    //     setIsModalOpen(false);
    //     setNewTask({ title: "", description: "", dueDate: null });
    // };

    // const handleAddTask = async () => {
    //     await dispatch(addTask({ task: newTask, stageId: id }));
    //     setIsModalOpen(false);
    //     setNewTask({ title: "", description: "", dueDate: null });
    // };
    ...
    return (
        <section className='task_column' key={id}>
            <div className='task_column_heading'>
                <p>{title}</p>
                <button className="stage_delete" 
                    // onClick={() => handleDelete(id)}
                >
                    <img className='delete_icon' alt='' />
                </button>
            </div>
            ...
            <DropArea 
                onDrop={() => onDrop({ id, index: 0 }, { activeCard })} 
            />
            ...
            {/* {hasMore && (
                <button 
                    ...
                    className="load_more_button bg-blue-500 text-white p-2 rounded w-100"
                >
                    Load More
                </button>
            )}
...
// const StageColumn = ({ id, stage, stages, title, handleDelete, activeCard, setActiveCard, stageData, onDrop }) => {
